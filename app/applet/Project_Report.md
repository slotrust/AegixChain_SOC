# AegisX Cyber Security SOC - Project Report
...
3. **External Environment Proxy Constraints:** When testing from external unauthenticated clients against the preview URL, requests intercept a Cookie Check Challenge (e.g., returning Status: 200 containing HTML check pages) prior to reaching the Express backend API. This means attacks are thwarted inherently by the infrastructure's Identity Aware Proxy before even hitting the application. Therefore, brute force tests using libraries like `requests.post()` on live proxy URLs without browser session cookies falsely report a "successful" connection (`Status: 200`) because they receive the proxy's HTML challenge, not the application's actual JSON backend response.
...
## Results Snapshots
* **Brute-Force Test (Local API Validation):**
  * Request 1 to 5: `Status: 401` - Invalid username or password
  * Request 6: `Status: 429` - Too many login attempts.
  * Request 7 to 20: `Status: 403` - Your IP address has been blocked.
* **Brute-Force Test (Remote Proxy Interaction Validation):** 
  * Returned HTTP 200 continuously. However, this is the AI Studio Identity-Aware Proxy intercepting the traffic and serving its `Cookie check` HTML challenge, effectively preventing the attacker from ever reaching the target backend API in the cloud environment.
